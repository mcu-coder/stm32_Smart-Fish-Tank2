...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagfeed)
    {
        bitFieldBitLen += feed_LEN;
    }
    if(0x01 == aFlag->flagJRP)
    {
        bitFieldBitLen += JRP_LEN;
    }
    if(0x01 == aFlag->flagbump1)
    {
        bitFieldBitLen += bump1_LEN;
    }
    if(0x01 == aFlag->flagbump2)
    {
        bitFieldBitLen += bump2_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagfeed)
    {
        info->event[info->num] = EVENT_feed;
        info->num++;
        dataPoints->valuefeed = gizVarlenDecompressionValue(bitFieldOffset,feed_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += feed_LEN;
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagJRP)
    {
        info->event[info->num] = EVENT_JRP;
        info->num++;
        dataPoints->valueJRP = gizVarlenDecompressionValue(bitFieldOffset,JRP_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += JRP_LEN;
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagbump1)
    {
        info->event[info->num] = EVENT_bump1;
        info->num++;
        dataPoints->valuebump1 = gizVarlenDecompressionValue(bitFieldOffset,bump1_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += bump1_LEN;
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagbump2)
    {
        info->event[info->num] = EVENT_bump2;
        info->num++;
        dataPoints->valuebump2 = gizVarlenDecompressionValue(bitFieldOffset,bump2_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += bump2_LEN;
    }
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtempvalue_MAX)
    {
        info->event[info->num] = EVENT_tempvalue_MAX;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetempvalue_MAX,(uint8_t *)&issuedData[byteFieldOffset],tempvalue_MAX_LEN);
        byteFieldOffset += tempvalue_MAX_LEN;
        dataPoints->valuetempvalue_MAX = gizX2Y(tempvalue_MAX_RATIO,  tempvalue_MAX_ADDITION, elongateP0FormTmp.devStatus.valuetempvalue_MAX); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtempvalue_MIN)
    {
        info->event[info->num] = EVENT_tempvalue_MIN;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetempvalue_MIN,(uint8_t *)&issuedData[byteFieldOffset],tempvalue_MIN_LEN);
        byteFieldOffset += tempvalue_MIN_LEN;
        dataPoints->valuetempvalue_MIN = gizX2Y(tempvalue_MIN_RATIO,  tempvalue_MIN_ADDITION, elongateP0FormTmp.devStatus.valuetempvalue_MIN); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagwaterlevelValue_MAX)
    {
        info->event[info->num] = EVENT_waterlevelValue_MAX;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuewaterlevelValue_MAX,(uint8_t *)&issuedData[byteFieldOffset],waterlevelValue_MAX_LEN);
        byteFieldOffset += waterlevelValue_MAX_LEN;
        dataPoints->valuewaterlevelValue_MAX = gizX2Y(waterlevelValue_MAX_RATIO,  waterlevelValue_MAX_ADDITION, elongateP0FormTmp.devStatus.valuewaterlevelValue_MAX); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagwaterlevelValue_MIN)
    {
        info->event[info->num] = EVENT_waterlevelValue_MIN;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuewaterlevelValue_MIN,(uint8_t *)&issuedData[byteFieldOffset],waterlevelValue_MIN_LEN);
        byteFieldOffset += waterlevelValue_MIN_LEN;
        dataPoints->valuewaterlevelValue_MIN = gizX2Y(waterlevelValue_MIN_RATIO,  waterlevelValue_MIN_ADDITION, elongateP0FormTmp.devStatus.valuewaterlevelValue_MIN); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagturbValue)
    {
        info->event[info->num] = EVENT_turbValue;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueturbValue,(uint8_t *)&issuedData[byteFieldOffset],turbValue_LEN);
        byteFieldOffset += turbValue_LEN;
        dataPoints->valueturbValue = gizX2Y(turbValue_RATIO,  turbValue_ADDITION, elongateP0FormTmp.devStatus.valueturbValue); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagtimeValue)
    {
        info->event[info->num] = EVENT_timeValue;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuetimeValue,(uint8_t *)&issuedData[byteFieldOffset],timeValue_LEN);
        byteFieldOffset += timeValue_LEN;
        dataPoints->valuetimeValue = gizX2Y(timeValue_RATIO,  timeValue_ADDITION, elongateP0FormTmp.devStatus.valuetimeValue); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuefeed != cur->valuefeed)
    {
        GIZWITS_LOG("valuefeed Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagfeed = 1;
        ret = 1;
    }
    if(last->valueJRP != cur->valueJRP)
    {
        GIZWITS_LOG("valueJRP Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagJRP = 1;
        ret = 1;
    }
    if(last->valuebump1 != cur->valuebump1)
    {
        GIZWITS_LOG("valuebump1 Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagbump1 = 1;
        ret = 1;
    }
    if(last->valuebump2 != cur->valuebump2)
    {
        GIZWITS_LOG("valuebump2 Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagbump2 = 1;
        ret = 1;
    }
    ...
    if(last->valuetempvalue_MAX != cur->valuetempvalue_MAX)
    {
        GIZWITS_LOG("valuetempvalue_MAX Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtempvalue_MAX = 1;
        ret = 1;
    }
    if(last->valuetempvalue_MIN != cur->valuetempvalue_MIN)
    {
        GIZWITS_LOG("valuetempvalue_MIN Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtempvalue_MIN = 1;
        ret = 1;
    }
    if(last->valuewaterlevelValue_MAX != cur->valuewaterlevelValue_MAX)
    {
        GIZWITS_LOG("valuewaterlevelValue_MAX Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagwaterlevelValue_MAX = 1;
        ret = 1;
    }
    if(last->valuewaterlevelValue_MIN != cur->valuewaterlevelValue_MIN)
    {
        GIZWITS_LOG("valuewaterlevelValue_MIN Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagwaterlevelValue_MIN = 1;
        ret = 1;
    }
    if(last->valueturbValue != cur->valueturbValue)
    {
        GIZWITS_LOG("valueturbValue Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagturbValue = 1;
        ret = 1;
    }
    if(last->valuetimeValue != cur->valuetimeValue)
    {
        GIZWITS_LOG("valuetimeValue Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagtimeValue = 1;
        ret = 1;
    }
    ...
    if(last->valuewaterleve != cur->valuewaterleve)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuewaterleve Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagwaterleve = 1;
            ret = 1;
        }
    }
    if(last->valueturb != cur->valueturb)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueturb Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagturb = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagfeed)
    {
        gizVarlenCompressValue(bitFieldOffset,feed_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuefeed);
        bitFieldOffset += feed_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagJRP)
    {
        gizVarlenCompressValue(bitFieldOffset,JRP_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueJRP);
        bitFieldOffset += JRP_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagbump1)
    {
        gizVarlenCompressValue(bitFieldOffset,bump1_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuebump1);
        bitFieldOffset += bump1_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagbump2)
    {
        gizVarlenCompressValue(bitFieldOffset,bump2_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valuebump2);
        bitFieldOffset += bump2_LEN;
    }
    ...
    {
        devStatusTmp.valuetime = gizY2X(time_RATIO,  time_ADDITION, dataPoints->valuetime); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetime,time_LEN);
        byteFieldOffset += time_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagtempvalue_MAX)
    {
        devStatusTmp.valuetempvalue_MAX = gizY2X(tempvalue_MAX_RATIO,  tempvalue_MAX_ADDITION, dataPoints->valuetempvalue_MAX); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetempvalue_MAX,tempvalue_MAX_LEN);
        byteFieldOffset += tempvalue_MAX_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagtempvalue_MIN)
    {
        devStatusTmp.valuetempvalue_MIN = gizY2X(tempvalue_MIN_RATIO,  tempvalue_MIN_ADDITION, dataPoints->valuetempvalue_MIN); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetempvalue_MIN,tempvalue_MIN_LEN);
        byteFieldOffset += tempvalue_MIN_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagwaterlevelValue_MAX)
    {
        devStatusTmp.valuewaterlevelValue_MAX = gizY2X(waterlevelValue_MAX_RATIO,  waterlevelValue_MAX_ADDITION, dataPoints->valuewaterlevelValue_MAX); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuewaterlevelValue_MAX,waterlevelValue_MAX_LEN);
        byteFieldOffset += waterlevelValue_MAX_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagwaterlevelValue_MIN)
    {
        devStatusTmp.valuewaterlevelValue_MIN = gizY2X(waterlevelValue_MIN_RATIO,  waterlevelValue_MIN_ADDITION, dataPoints->valuewaterlevelValue_MIN); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuewaterlevelValue_MIN,waterlevelValue_MIN_LEN);
        byteFieldOffset += waterlevelValue_MIN_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagturbValue)
    {
        devStatusTmp.valueturbValue = gizY2X(turbValue_RATIO,  turbValue_ADDITION, dataPoints->valueturbValue); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueturbValue,turbValue_LEN);
        byteFieldOffset += turbValue_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagtimeValue)
    {
        devStatusTmp.valuetimeValue = gizY2X(timeValue_RATIO,  timeValue_ADDITION, dataPoints->valuetimeValue); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetimeValue,timeValue_LEN);
        byteFieldOffset += timeValue_LEN;
    }

    if(gizwitsProtocol.waitReportDatapointFlag.flagwaterleve)
    {
        devStatusTmp.valuewaterleve = exchangeBytes(gizY2X(waterleve_RATIO,  waterleve_ADDITION, dataPoints->valuewaterleve)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuewaterleve,waterleve_LEN);
        byteFieldOffset += waterleve_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagturb)
    {
        devStatusTmp.valueturb = exchangeBytes(gizY2X(turb_RATIO,  turb_ADDITION, dataPoints->valueturb)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueturb,turb_LEN);
        byteFieldOffset += turb_LEN;
    }
